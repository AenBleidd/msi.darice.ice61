...
While Not recinfo Is Nothing
    If (Len(recinfo.StringData(colVersionMax)) > 0) And (Len(recinfo.StringData(colVersionMin)) > 0) Then
        ...
        If VersionFormatVerification(recinfo.StringData(colVersionMax)) = -1 Then
            ...
            ICE61 = 1
        End If
        If VersionFormatVerification(recinfo.StringData(colVersionMin)) = -1 Then
            bWrongVersionFormat = True
            ...
            ICE61 = 1
        End If
        If Not bWrongVersionFormat And CompareVersions(recinfo.StringData(colVersionMax), recinfo.StringData(colVersionMin)) < 0 Then
            ...
            Message &H3000000, recinfo
            ICE61 = 1
		ElseIf Not bWrongVersionFormat And (CompareVersions(recinfo.StringData(colVersionMax), recinfo.StringData(colVersionMin)) = 0) AND Not ((recinfo.integerdata(colAttributes) And UPGRADE_ATTRIBUTE_ALLINCLUSIVE) = UPGRADE_ATTRIBUTE_ALLINCLUSIVE) Then
...
While Not recinfo Is Nothing
    If Not ((recinfo.integerdata(colAttributes) And UPGRADE_ATTRIBUTE_DETECTONLY) = UPGRADE_ATTRIBUTE_DETECTONLY) Then
        If Len(recinfo.StringData(colVersionMax)) Then
            If (((recinfo.integerdata(colAttributes) And UPGRADE_ATTRIBUTE_MAXINCLUSIVE) And CompareVersions(recinfo.StringData(colVersionMax), sPV) >= 0) Or ((Not (recinfo.integerdata(colAttributes) And UPGRADE_ATTRIBUTE_MAXINCLUSIVE) And CompareVersions(recinfo.StringData(colVersionMax), sPV) > 0))) Then
...
On Error Resume Next
  ...
  If sTemp <> "" Then
	  ...
	  If sTemp <> "" Then
		  iPos = InStr(sTemp & ".", ".")
		  If CLng(Left(sTemp, iPos - 1)) > 65535 Then VersionStringToLong = -1: Exit Function
		  sAccum = sAccum & Right("0000" & Hex(CLng(Left(sTemp, iPos - 1))), 4)
		  If Err.Number > 0 Then VersionStringToLong = -1: Exit Function
	  End If
  End If
...
'Returns -1 if the version number format is wrong
'Return 0 if the version number format is correct
Function VersionFormatVerification(strng)
Dim i, iPos, sTemp
On Error Resume Next
  sTemp = strng
  While Left(sTemp, 1) <> ""
    If InStr("0123456789.", Left(sTemp, 1)) = 0 Then VersionFormatVerification = -1: Exit Function
    sTemp = Mid(sTemp, 2)
  Wend
  sTemp = strng
  iPos = InStr(sTemp & ".", ".")
  If CLng(Left(sTemp, iPos - 1)) > 255 Then VersionFormatVerification = -1: Exit Function
  If Err.Number > 0 Then VersionFormatVerification = -1: Exit Function
  sTemp = Mid(sTemp & ".", iPos + 1)
  If sTemp <> "" Then
	  iPos = InStr(sTemp & ".", ".")
	  If CLng(Left(sTemp, iPos - 1)) > 255 Then VersionFormatVerification = -1: Exit Function
	  ...
	  If sTemp <> "" Then
		  iPos = InStr(sTemp & ".", ".")
		  If CLng(Left(sTemp, iPos - 1)) > 65535 Then VersionFormatVerification = -1: Exit Function
		  If Err.Number > 0 Then VersionFormatVerification = -1: Exit Function
	  End If
  End If
  VersionFormatVerification = 0
  Exit Function
End Function

'Returns 0 if the version numbers are equal
'Returns 1 if the first version number is greater
'Returns -1 if the first version number is less
Function CompareVersions(strng1, strng2)
Dim i, iPos, sTemp, major1, minor1, build1, major2, minor2, build2
On Error Resume Next
  major1 = 0
  minor1 = 0
  build1 = 0
  ...
  sTemp = strng1
  iPos = InStr(sTemp & ".", ".")
  major1 = CLng(Left(sTemp, iPos - 1))
  sTemp = Mid(sTemp & ".", iPos + 1)
  If sTemp <> "" Then
	  iPos = InStr(sTemp & ".", ".")
    minor1 = CLng(Left(sTemp, iPos - 1))
	  sTemp = Mid(sTemp & ".", iPos + 1)
	  If sTemp <> "" Then
		  iPos = InStr(sTemp & ".", ".")
      build1 = CLng(Left(sTemp, iPos - 1))
	  End If
  End If

  sTemp = strng2
  iPos = InStr(sTemp & ".", ".")
  major2 = CLng(Left(sTemp, iPos - 1))
  sTemp = Mid(sTemp & ".", iPos + 1)
  If sTemp <> "" Then
	  iPos = InStr(sTemp & ".", ".")
    minor2 = CLng(Left(sTemp, iPos - 1))
	  sTemp = Mid(sTemp & ".", iPos + 1)
	  If sTemp <> "" Then
		  iPos = InStr(sTemp & ".", ".")
      build2 = CLng(Left(sTemp, iPos - 1))
	  End If
  End If
